...
void Labels::update(const View& _view, float _dt, const std::vector<std::unique_ptr<Style>>& _styles,
...
#if 1
    {
        ...
        if (narrowTime + broadTime > 0.5f) {
            logMsg("new: %d labels : %f %f\n", m_labels.size(), broadTime, narrowTime);
        }
    }
...
#if 1
        {
            auto pairs = isect2d::intersect(m_aabbs, {4, 4}, {_view.getWidth(), _view.getHeight()});
