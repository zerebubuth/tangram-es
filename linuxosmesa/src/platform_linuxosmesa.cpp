...
bool startUrlRequest(const std::string& _url, UrlCallback _callback) {
    ...
    for(auto& worker : s_Workers) {
        if(worker.isAvailable()) {
          std::cout << "Started request for " << _url << std::endl;
...
void initGLExtensions() {
     glBindVertexArrayOESEXT = (PFNGLBINDVERTEXARRAYPROC)OSMesaGetProcAddress("glBindVertexArray");
     glDeleteVertexArraysOESEXT = (PFNGLDELETEVERTEXARRAYSPROC)OSMesaGetProcAddress("glDeleteVertexArrays");
     glGenVertexArraysOESEXT = (PFNGLGENVERTEXARRAYSPROC)OSMesaGetProcAddress("glGenVertexArrays");

}

bool tilesLoading() {
  bool taskQueueEmpty = s_urlTaskQueue.empty();
  bool allWorkersAvailable = true;
  for (auto& worker : s_Workers) {
    if (!worker.isAvailable()) {
      allWorkersAvailable = false;
    }
  }
  std::cout << "tilesLoading: taskQueueEmpty: " << taskQueueEmpty << ", allWorkersAvailable: " << allWorkersAvailable << std::endl;

  if (!taskQueueEmpty) {
    return true;
  }
  return !allWorkersAvailable;
}
